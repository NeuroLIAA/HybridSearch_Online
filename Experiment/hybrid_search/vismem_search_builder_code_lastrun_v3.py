...
expInfo = {'participant': '1', 'session': '001', 'gender': 'f', 'age': '30'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/lpzmi/Dropbox/Notts/third_year_projects/2019_2020/Dounia_Adeishe/Psychopy_vismemsearch/vismem_search_builder_code_lastrun.py',
...
win = visual.Window(
    ...
    monitor='mac_office', color=[0,0,0], colorSpace='rgb',
...
for thisTrial in trials:
    ...
    dist8.setImage(D8)
    # keep track of which components have finished
    trialComponents = [target, dist1, dist2, dist3, dist4, dist5, dist6, dist7, dist8]
    ...
    # update component parameters for each repeat
    search_img.setOpacity(0.5)
    search_img.setImage(FILE_SEARCH_IMAGE)
    key_resp.keys = []
    key_resp.rt = []
    # keep track of which components have finished
    searchComponents = [search_img, key_resp]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if search_img.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > search_img.tStartRefresh + 6-frameTolerance:
                # keep track of stop time/frame for later
...
msg = "Thanks for participating. Your performance was %.2f (rt=%.2f)" %(Perfo,meanRt)
text.setText(msg)
# keep track of which components have finished
